...
# Create necessary configuration directory and file if not already present
CONFIG_DIR="$HOME/.config/hyprupld"
CONFIG_FILE="$CONFIG_DIR/uploader.json"
mkdir -p "$CONFIG_DIR"

# Initialize configuration file if it does not exist
if [ ! -f "$CONFIG_FILE" ]; then
  echo "{\"uploader\": \"$DEFAULT_UPLOADER\"}" > "$CONFIG_FILE"
fi

# Function to read the current uploader choice from configuration
get_uploader() {
  jq -r '.uploader' "$CONFIG_FILE"
}

# Function to update the uploader choice in configuration
set_uploader() {
  local new_uploader=$1
  echo "{\"uploader\": \"$new_uploader\"}" > "$CONFIG_FILE"
}

# Check for the -upldr flag to change uploader
if [[ "$1" == "-upldr" ]]; then
  # Use Zenity to display a selection dialog
  choice=$(zenity --list --title="Select Uploader" \
    --text="Choose your preferred uploader:" \
    --radiolist \
    --column="Select" --column="Uploader" \
    TRUE "e-z.gg" FALSE "guns.lol")
  
  # Check the user's choice
  if [[ "$choice" == "e-z.gg" ]]; then
    set_uploader "e-z.gg"
    zenity --info --text="Uploader set to e-z.gg"
  elif [[ "$choice" == "guns.lol" ]]; then
    set_uploader "guns.lol"
    zenity --info --text="Uploader set to guns.lol"
  else
    zenity --error --text="Invalid choice or no selection made. No changes applied."
  fi

  exit 0
fi

# Retrieve the selected uploader
SELECTED_UPLOADER=$(get_uploader)

# Run the appropriate script based on the selected uploader
case $SELECTED_UPLOADER in
  "e-z.gg")
    # e-z.gg script
    echo "Running e-z.gg..."
    # BEGIN e-z.gg
...
rm -f "$temp_file" "$response_file"
    ...
    ;;
  *)
    echo "Invalid uploader in configuration. Please run with -upldr to set it."
    exit 1
    ;;
esac
echo "Script by PhoenixAceVFX"
echo "Licensed under GPL-2.0"
